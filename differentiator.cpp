...
struct Node_t* Derivative_add(Node_t* node) 
{
    ...
    // struct Node_t* main_node = Build_node (_ADD, "@", 0, l, r, OPERATION);

    // return main_node; 

    return Build_node (_ADD, "@", 0, 
                        Differentiator (node->left), 
                        Differentiator (node->right),
                        OPERATION);
...
struct Node_t* Derivative_sub (Node_t* node) 
{
    ...
    struct Node_t* main_node = Build_node( _SUB, "@", 0, l, r, OPERATION);
    return main_node; 
}
...
struct Node_t* Derivative_mul (Node_t* node)
{
    ...
    struct Node_t* l =  Build_node ( _MUL, "@", 0, r_l, r_r, OPERATION);

    struct Node_t* r =  Build_node ( _MUL, "@", 0, l_l, l_r, OPERATION);
...
struct Node_t* Derivative_div (Node_t* node)
{
    ...
    struct Node_t* l_r_r = Copy_tree (node->left);
    ...
    struct Node_t* r_l = Copy_tree (node->right);
    ...
    struct Node_t* main_node = Build_node (_DIV, "@", 0, l, r, OPERATION);
    return main_node;
}
...
struct Node_t* Derivative_sin (Node_t* node)
{
    ...
    struct Node_t* r = Differentiator (node->right);
    struct Node_t* l = Build_node (_COS, "@", 0, l_l, l_r, OPERATION);
...
struct Node_t* Derivative_cos (Node_t* node)
{
    ...
    struct Node_t* l_r = Build_node (_SIN, "@", 0, l_r_l, l_r_r, OPERATION);
    struct Node_t* l_l = Build_node ( _NOTHING, "@", 0, NULL, NULL, EMPTINESS);
