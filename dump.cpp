...
void Print_struct_Node (struct Node_t* Node, FILE* file)
{
    ...
    if (Node->type == X)
    {
        ...
        fprintf (file, "%d[width=2, shape=record, label= \"{ip: %p| type: %d| value: %s | {<f1> L | <f2> R}}\"," 
        "style=filled, fillcolor=\"lightcyan\", color=\"cyan\"];\n", 
    ...
    if (Node->type == OPERATION)
    {
        ...
        fprintf (file, "%d[width=2, shape=record, label= \"{ip: %p| type: %d| value: %s | {<f1> L | <f2> R}}\","
        "style=filled, fillcolor=\"lightgreen\", color=\"green\"];\n", 
    ...
    if (Node->type == NUMBER)
    {
        ...
        fprintf (file, "%d[width=2, shape=record, label= \"{ip: %p| type: %d| value: %.2lf| {<f1> L | <f2> R}}\","
        "style=filled, fillcolor=\"lightpink\", color=\"pink\"];\n", 
    ...
    {
        fprintf (file, "%d[width=2, shape=record, label= \"{ip: %p| type: %d| value: %.2lf| {<f1> L | <f2> R}}\","
        "style=filled, fillcolor=\"lightyellow\", color=\"orange\"];\n", 
        &(Node->elem), &(Node->elem), Node->type, Node->elem.number);
    }
